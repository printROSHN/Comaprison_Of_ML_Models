...
def encode(data,target):
    ...
    # Apply the same target encoding to the target column for consistency
    enc = get_single_encoder("OrdinalEncoder", [target])
    encoded_target_col = enc.fit_transform(data[target])  # Note: Fit on the entire data, not just target_col_data
    print("\nEncoded target: ", encoded_target_col)
    merge_list.append(encoded_target_col)
    print(encoded_target_col.shape)
    
    # seperating date-time cols
    # date_time = seprate_date_time(non_target_data)
    # print(date_time)
    # merge_list.append(date_time)
    # non_target_data = non_target_data.drop(date_time, axis=1)
